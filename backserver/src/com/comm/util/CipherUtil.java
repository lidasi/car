...
public class CipherUtil {  
     ...
     * @return 返回加密后的摘要信息 
     */ 
    private static byte[] encryptEncode(String algorithm, byte[] data) {
        try {  
            MessageDigest md = MessageDigest.getInstance(algorithm);  
     ...
     * @return 返回密钥信息 
     */  
    public static byte[] generateMACKeyBytes() {  
        try {  
            KeyGenerator keyGenerator = KeyGenerator.getInstance(ALGORITHM_MAC);  
            SecretKey secretKey = keyGenerator.generateKey();  
     ...
     * @return 返回密钥字节数组
     */  
    public static byte[] generateDESKey(byte[] seed) {  
        try {  
            KeyGenerator kg = KeyGenerator.getInstance(ALGORITHM_DES);  
     ...
     * @return 返回加密或解密的数据 
     */  
    private static byte[] DESCipher(byte[] data, byte[] key, int mode) {  
        try {  
            Key k = toKey(key, ALGORITHM_DES);  
            Cipher cipher = Cipher.getInstance(ALGORITHM_DES);  
            cipher.init(mode, k);  
     ...
     * @return 返回盐信息 
     */  
    public static byte[] generatePBESaltBytes() {  
        byte[] salt = new byte[8];  
        Random random = new Random();  
        random.nextBytes(salt);  
        return salt;  
    ...
    /** 
     * PBE(Password-based encryption基于密码加密)加密 （字符串）
     * @param data 要加密的数据 
     * @param password 密码 
     * @param salt 盐 
     * @return 返回加密后的数据(经过base64编码) 
    ...
    /** 
     * PBE(Password-based encryption基于密码加密)加密 （字节数组）
     * @param data 要加密的数据 
     * @param password 密码 
     * @param salt 盐 
     * @return 返回加密后的数据(经过base64编码) 
     ...
     * @return 返回加密解密后的数据 
     */  
    private static byte[] PBECipher(byte[] data, String password, byte[] salt, int mode) {  
        try {  
            Key secretKey = toPBEKey(password);  
     ...
     * @return 返回base64编码后的key信息 
     */
    public static byte[] generateAESKeyBytes(int keySize, String seed) {
        try {
            KeyGenerator kgen = KeyGenerator.getInstance(KEY_AES);
            kgen.init(keySize, new SecureRandom(seed.getBytes()));
            SecretKey key = kgen.generateKey();
     ...
     * @return 返回base64编码后的key信息 
     */
    public static String generateAESKeyHexstr(int keySize, String seed) {
        try {
            KeyGenerator kgen = KeyGenerator.getInstance(KEY_AES);
            kgen.init(keySize, new SecureRandom(seed.getBytes()));
            SecretKey key = kgen.generateKey();
     ...
     * @return 返回加密数据 
     */  
    public static byte[] AESEncrypt(byte[] data, byte[] key, String algorithmParameter) {  
        return AESCipher(data, key, algorithmParameter, Cipher.ENCRYPT_MODE);  
    }  
     ...
     * @return 返回解密数据 
     */  
    public static byte[] AESDecrypt(byte[] data, byte[] key, String algorithmParameter) {  
        return AESCipher(data, key, algorithmParameter,Cipher.DECRYPT_MODE);  
    } 
     ...
     * @return 返回加密或解密的数据 
     */
    private static byte[] AESCipher(byte[] data, byte[] key, String algorithmParameter, int mode) {
        try {
            Key k = toKey(key,KEY_AES);
            AlgorithmParameterSpec paramSpec = new IvParameterSpec(algorithmParameter.getBytes());
            Cipher ecipher = Cipher.getInstance(ALGORITHM_AES);
    ...
    public static byte[] AESEncrypt(byte[] data, String keyString) {
    	try {
			byte[] byteKey = TranscodeUtil.hexStrToByteArray(keyString);
     ...
     * @return 返回解密数据 
     */  
    public static byte[] AESDecrypt(byte[] data, String keyString) {
		try {
			byte[] byteKey = TranscodeUtil.hexStrToByteArray(keyString);
			SecretKeySpec key = new SecretKeySpec(byteKey, "AES");
			Cipher cipher = Cipher.getInstance("AES");
